...
if [ -z ${CONDA_PATH+x} ]; then
  ...
  ./install.sh --env-name=mirgeDriver.Y1nozzle-lazy $git_method --branch=production
else
  ...
  ./install.sh --conda-prefix=$CONDA_PATH --env-name=mirgeDriver.Y1nozzle-lazy $git_method --branch=production
fi
