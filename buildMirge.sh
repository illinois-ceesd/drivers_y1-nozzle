...
if [ -z ${CONDA_PATH+x} ]; then
  ...
  echo "./install.sh --env-name=mirgeDriver.Y1nozzle $git_method --branch=production-state-handling"
  ./install.sh --env-name=mirgeDriver.Y1nozzle-lazy $git_method --branch=production-state-handling
else
  echo "Using existing Conda installation, ${CONDA_PATH}"
  echo "./install.sh --conda-prefix=$CONDA_PATH --env-name=mirgeDriver.Y1nozzle $git_method --branch=production-state-handling"
  ./install.sh --conda-prefix=$CONDA_PATH --env-name=mirgeDriver.Y1nozzle-lazy $git_method --branch=production-state-handling
fi
